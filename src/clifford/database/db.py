...
class Registry:
    ...
    def _initialize_database(self) -> None:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("CREATE INDEX IF NOT EXISTS idx_experiment_runs_experiment_id ON experiment_runs(experiment_id)")
    ...
    def create_experiment(self, name: str, description: Optional[str] = None, tags: Optional[List[str]] = None) -> str:
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO experiments (id, name, description, tags, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
                (experiment_id, name, description, json.dumps(tags or []), now, now)
            )
            conn.commit()
    ...
    def get_experiment(self, experiment_id: str) -> Optional[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, name, description, tags, created_at, updated_at FROM experiments WHERE id = ?", (experiment_id,))
            row = cursor.fetchone()
    ...
    def list_experiments(self) -> List[Dict[str, Any]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("SELECT id, name, description, tags, created_at, updated_at FROM experiments")
            rows = cursor.fetchall()
    ...
    def create_experiment_run(self, experiment_id: str, model_id: Optional[str] = None, config: Optional[Dict[str, Any]] = None) -> str:
        run_id = str(uuid.uuid4())
        now = datetime.now().isoformat()
        ...
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO experiment_runs (id, experiment_id, model_id, config, start_time, status) VALUES (?, ?, ?, ?, ?, ?)",
                (run_id, experiment_id, model_id, json.dumps(config or {}), now, "running")
            )
            conn.commit()
        
        return run_id

    def update_experiment_run(self, run_id: str, status: str, results: Optional[Dict[str, Any]] = None) -> None:
        now = datetime.now().isoformat()
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if status == "completed":
                cursor.execute(
                    "UPDATE experiment_runs SET end_time = ?, status = ?, results = ? WHERE id = ?",
                    (now, status, json.dumps(results or {}), run_id)
                )
            else:
                cursor.execute(
